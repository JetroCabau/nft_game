...
import PickleRenderer from "./components/PickleRenderer";
...
function App() {
  ...
  const levelUpPickle = (_account, _id) => {
    setLoading(true);
    blockchain.PickleCoin.methods
      .levelUp(_id)
      .send({
      ...
      {blockchain.account === "" || blockchain.PickleCoin === null ? (
        ...
        <s.Container ai={"center"} style={{ padding: "24px" }}>
          ...
          <s.Container jc={"center"} fd={"row"} style={{ flexWrap: "wrap" }}>
            {data.allPickle.map((item, index) => {
              return (
                <s.Container key={index} style={{ padding: "15px" }}>
                  <PickleRenderer Pickle={item} />
                  <s.SpacerXSmall />
                  <s.Container>
                    ...
                    <button
                      ...
                      onClick={(e) => {
                        e.preventDefault();
                        levelUpPickle(blockchain.account, item.id);
                      }}
                    >
                      Level Up
                    </button>
